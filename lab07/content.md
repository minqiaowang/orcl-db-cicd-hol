...
We have a web application developed in Python, running as a micro-service in a Docker container, on OKE cluster using OCI. This application connects to Oracle Database, and we have seen how CI/CD principles and concepts can be applied to develop this application. Now we will apply same principles to Oracle Database changes, when we need to alter existing objects, or create new ones, while working on application features. At the same time, we have to track these database object changes, merge them with the master branch, and make them available to other developers.

This is why we need a tool to track, version, and deploy database schema changes - [Liquibase](https://www.liquibase.org/). Liquibase supports a broad range of databases, including Oracle Database. Access the website, and find the URL for the latest stable release for Linux x64.

## Install Liquibase

Deactivate Python virtual environment, and go to oracle's home.
...
## Use Liquibase on Development for CI/CD
